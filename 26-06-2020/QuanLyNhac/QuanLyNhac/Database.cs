...
namespace QuanLyNhac
{
    class Database
    {
                ...
                try
                {
                    ...
                    {                        
                        ...
                        songs.Add(new Song()
                        {
                            ...
                            GenreID = Convert.ToInt32(dataReader.GetValue(3)),
                        });
                    }
                    dataReader.Close();
                    command.Dispose();
        ...
        public Boolean deleteSong(Song selectedSong)
        {
            Boolean result = true;
            using (SqlConnection connection = this.GetConnection())
            {
                try
                {
                    string sql = "DELETE FROM Songs WHERE Songs.ID = @SongId;";
                    SqlCommand command = new SqlCommand(sql, connection);
                    command.Parameters.Add("@SongId", SqlDbType.Int).Value = selectedSong.ID;                    
                    command.ExecuteNonQuery();
                    command.Dispose();
                    connection.Close();
                }
